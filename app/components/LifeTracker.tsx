...
// ─── Types ────────────────────────────────────────────────────────────────────
type CategoryId = "move" | "explore" | "relax" | "nourish" | "connect";
...
type Habit = {
  ...
  type: HabitType; category: CategoryId;
  frequency: Frequency; active: boolean;
  allowComments: boolean;
};

type HabitLog = {
  id: string; habitId: string;
  value: string; comment?: string; date: string;
};

type DayRating = {
  id: string; date: string;
  ratings: Record<CategoryId, Rating>;
  notes: Record<CategoryId, string>;
};

type Meal = {
  id: string; mealType: string; description: string;
  tag: MealTag; portion: Portion;
  calories?: number; time: string; date: string; isBeverage?: boolean;
};

type CalorieItem = {
  id: string; name: string; emoji: string;
  calories: number; category: string; isCustom?: boolean;
};
...
const CATEGORIES: Record<CategoryId, { label: string; emoji: string; color: string; bg: string; description: string }> = {
  move:    { label: "MOVE",    emoji: "💪", color: "#ff6b6b", bg: "#fff0f0", description: "Activity & fitness" },
  explore: { label: "EXPLORE", emoji: "🧠", color: "#a78bfa", bg: "#f3f0ff", description: "Learning & growth" },
  relax:   { label: "RELAX",   emoji: "😌", color: "#4ecdc4", bg: "#e8faf9", description: "Rest & mindfulness" },
  nourish: { label: "NOURISH", emoji: "🥗", color: "#4ade80", bg: "#f0fdf4", description: "Food & hydration" },
  connect: { label: "CONNECT", emoji: "🤝", color: "#f5c842", bg: "#fdf3c0", description: "Family & relationships" },
};
...
const FREQUENCY_LABELS: Record<Frequency, string> = {
  ...
  weekly: "Weekly", weekdays: "Weekdays", weekends: "Weekends",
};
...
const PORTION_CONFIG: Record<Portion, string> = {
  half: "½", single: "1×", double: "2×", triple: "3×",
};

const DEFAULT_HABITS: Habit[] = [
  { id: "h1",  title: "Morning brush",     emoji: "🪥", type: "checkbox", category: "relax",   frequency: "daily",   active: true, allowComments: false },
  { id: "h2",  title: "Night brush",       emoji: "🪥", type: "checkbox", category: "relax",   frequency: "daily",   active: true, allowComments: false },
  { id: "h3",  title: "Walking",           emoji: "🚶", type: "checkbox", category: "move",    frequency: "daily",   active: true, allowComments: true  },
  { id: "h4",  title: "Tennis",            emoji: "🎾", type: "checkbox", category: "move",    frequency: "2x_week", active: true, allowComments: true  },
  { id: "h5",  title: "Strength training", emoji: "🏋️", type: "checkbox", category: "move",    frequency: "2x_week", active: true, allowComments: true  },
  { id: "h6",  title: "Play with kids",    emoji: "👨‍👧‍👦", type: "checkbox", category: "connect", frequency: "daily",   active: true, allowComments: true  },
  { id: "h7",  title: "Bedtime",           emoji: "😴", type: "time",     category: "relax",   frequency: "daily",   active: true, allowComments: false },
  { id: "h8",  title: "Read",              emoji: "📚", type: "checkbox", category: "explore", frequency: "daily",   active: true, allowComments: true  },
  { id: "h9",  title: "Learn something",  emoji: "🎯", type: "checkbox", category: "explore", frequency: "3x_week", active: true, allowComments: true  },
  { id: "h10", title: "Call family",       emoji: "📞", type: "checkbox", category: "connect", frequency: "weekly",  active: true, allowComments: true  },
];
...
const S: Record<string, React.CSSProperties> = {
  ...
  sectionTitle: { fontSize: 12, fontWeight: 700, color: C.muted, textTransform: "uppercase" as const, letterSpacing: "0.5px", marginBottom: 8 },
};

const btn = (variant = "primary", extra?: React.CSSProperties): React.CSSProperties => ({
  background: variant === "primary" ? C.gold : variant === "danger" ? C.coral : variant === "navy" ? C.navy : "transparent",
  ...
  cursor: "pointer", display: "inline-flex" as const, alignItems: "center" as const, gap: 6,
...
const navBtn = (active: boolean): React.CSSProperties => ({
  ...
  color: active ? C.navy : C.muted, fontSize: 9, fontWeight: active ? 800 : 500,
...
function getFrequencyTarget(freq: Frequency): number {
  ...
  if (freq === "2x_week") return 2;
  if (freq === "3x_week") return 3;
  if (freq === "weekly") return 1;
  if (freq === "weekdays") return 5;
  if (freq === "weekends") return 2;
  return 1;
}
  ...
  return (
    <div style={{ ...S.app, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh" }}>
      <div style={{ textAlign: "center", width: "100%", maxWidth: 360, padding: 24 }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 14, marginBottom: 32, transform: shake ? "translateX(8px)" : "none", transition: "transform 0.1s" }}>
          {[0,1,2,3].map(i => <div key={i} style={{ width: 20, height: 20, borderRadius: "50%", background: i < pin.length ? C.navy : C.border, transition: "background 0.15s" }} />)}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, maxWidth: 260, margin: "0 auto" }}>
          {[1,2,3,4,5,6,7,8,9,"",0,"⌫"].map((d, i) => (
            <button key={i} onClick={() => { if (d === "⌫") setPin(p => p.slice(0,-1)); else if (d !== "") handleKey(String(d)); }}
              style={{ padding: "16px 0", fontSize: d === "⌫" ? 20 : 22, fontWeight: 700, background: d === "" ? "transparent" : C.card, border: d === "" ? "none" : `1.5px solid ${C.border}`, borderRadius: 12, cursor: d === "" ? "default" : "pointer", color: C.navy, fontFamily: "inherit" }}>{d}</button>
          ))}
        </div>
      </div>
    </div>
...
// ─── Habit Log Form (inline) ──────────────────────────────────────────────────
function HabitLogForm({ habit, existing, onSave, onCancel }: {
  habit: Habit; existing?: HabitLog;
  onSave: (value: string, comment?: string) => void;
  onCancel: () => void;
}) {
  ...
  return (
    <div style={{ background: C.bg, borderRadius: 12, padding: 12, marginTop: 8, border: `1px solid ${C.border}` }}>
      {habit.type === "time" && (
        <div style={{ marginBottom: habit.allowComments ? 10 : 0 }}>
          <label style={S.label}>Time</label>
          <input style={S.input} type="time" value={time} onChange={e => setTime(e.target.value)} />
        </div>
      )}
      {habit.allowComments && (
        <div style={{ marginTop: habit.type === "time" ? 10 : 0 }}>
          <label style={S.label}>Comment (optional)</label>
          <input style={S.input} placeholder={`Notes on ${habit.title.toLowerCase()}...`} value={comment} onChange={e => setComment(e.target.value)} />
        </div>
      )}
      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
        <button style={{ ...btn("primary"), flex: 1, justifyContent: "center", padding: "9px" }}
          onClick={() => onSave(habit.type === "time" ? time : "done", comment || undefined)}>Save</button>
...
// ─── Today Tab ────────────────────────────────────────────────────────────────
function TodayTab({ habits, habitLogs, dayRating, onSaveLog, onRemoveLog, onGoCheckin }: {
  habits: Habit[]; habitLogs: HabitLog[]; dayRating?: DayRating;
  onSaveLog: (habitId: string, value: string, comment?: string) => void;
  onRemoveLog: (habitId: string) => void;
  onGoCheckin: () => void;
}) {
  ...
  const lifeScore = dayRating ? Math.round((Object.values(dayRating.ratings).reduce((s,r) => s+r,0)/5)*20) : null;

  function handleCheckbox(habit: Habit) {
    const log = getLog(habit.id);
    if (log) {
      // already done — if has comments, open form to edit, else just toggle off
      if (habit.allowComments) { setOpenForm(openForm === habit.id ? null : habit.id); }
      else { onRemoveLog(habit.id); }
    } else {
      // not done — if needs comment/time, open form, else instant check
      if (habit.allowComments) { setOpenForm(habit.id); }
      else { onSaveLog(habit.id, "done"); }
    }
  }
  ...
  return (
    <div>
      ...
      <div style={{ ...S.card, background: C.navy, color: "#fff", marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            {lifeScore !== null ? (
              <>
                <div style={{ fontSize: 42, fontWeight: 900, color: C.gold, lineHeight: 1 }}>{lifeScore}<span style={{ fontSize: 18 }}>/100</span></div>
                <div style={{ fontSize: 12, color: "#a0aec0", marginTop: 2 }}>Life Score today</div>
              </>
            ) : (
              <>
                <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 2 }}>No check-in yet</div>
                <div style={{ fontSize: 12, color: "#a0aec0" }}>Rate your day tonight</div>
              </>
            )}
          </div>
          <div style={{ textAlign: "right" as const }}>
            {dayRating && (
              <div style={{ display: "flex", flexDirection: "column", gap: 3, marginBottom: 8 }}>
                {(Object.entries(CATEGORIES) as [CategoryId, typeof CATEGORIES[CategoryId]][]).map(([id, cat]) => (
                  <div key={id} style={{ display: "flex", alignItems: "center", gap: 5 }}>
                    <span style={{ fontSize: 11 }}>{cat.emoji}</span>
                    <div style={{ display: "flex", gap: 2 }}>
                      {[1,2,3,4,5].map(n => <div key={n} style={{ width: 7, height: 7, borderRadius: "50%", background: n <= dayRating.ratings[id] ? cat.color : "rgba(255,255,255,0.2)" }} />)}
                    </div>
                  </div>
                ))}
              </div>
            )}
            <button onClick={onGoCheckin} style={{ ...btn("primary"), fontSize: 12, padding: "8px 14px" }}>
              {dayRating ? "✏️ Edit" : "⭐ Check-in"}
            </button>
          </div>
        </div>
        {/* Habit progress bar */}
      ...
      {/* Habits by category */}
      {(Object.entries(CATEGORIES) as [CategoryId, typeof CATEGORIES[CategoryId]][]).map(([catId, cat]) => {
        ...
        return (
          <div key={catId} style={{ marginBottom: 14 }}>
            ...
            {catHabits.map(habit => {
              const log = getLog(habit.id);
              const isDone = !!log;
              const isOpen = openForm === habit.id;
              const isWeekly = !["daily","weekdays","weekends"].includes(habit.frequency);
              const weekHits = isWeekly ? getWeeklyHits(habit, habitLogs) : 0;
              const target = getFrequencyTarget(habit.frequency);
                  ...
                  {habit.type === "checkbox" ? (
                    <div style={{ ...S.card, padding: "12px 14px", border: isDone ? `2px solid ${cat.color}` : `1px solid ${C.border}`, background: isDone ? cat.bg : C.card, marginBottom: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          {isWeekly && <div style={{ fontSize: 11, color: weekHits >= target ? cat.color : C.muted }}>{weekHits}/{target} this week · {FREQUENCY_LABELS[habit.frequency]}</div>}
                        </div>
                        {habit.allowComments && (
                          <button onClick={() => setOpenForm(isOpen ? null : habit.id)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16, color: C.muted, padding: 4 }}>
                            {isOpen ? "▲" : "💬"}
                          </button>
                        )}
                      </div>
                      {isOpen && (
                        <HabitLogForm habit={habit} existing={log}
                          onSave={(v, c) => { onSaveLog(habit.id, v, c); setOpenForm(null); }}
                    ...
                    <div style={{ ...S.card, padding: "12px 14px", border: isDone ? `2px solid ${cat.color}` : `1px solid ${C.border}`, background: isDone ? cat.bg : C.card, marginBottom: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        ...
                        <button onClick={() => setOpenForm(isOpen ? null : habit.id)} style={{ ...btn("navy"), fontSize: 12, padding: "8px 12px" }}>
                          {isDone ? "✏️ Edit" : "Log now"}
                        </button>
                      </div>
                      {isOpen && (
                        <HabitLogForm habit={habit} existing={log}
...
function CheckinTab({ existing, onSave }: { existing?: DayRating; onSave: (r: DayRating) => void }) {
  const [ratings, setRatings] = useState<Record<CategoryId, Rating>>(
    existing?.ratings || { move: 3, explore: 3, relax: 3, nourish: 3, connect: 3 }
  );
  const [notes, setNotes] = useState<Record<CategoryId, string>>(
    existing?.notes || { move: "", explore: "", relax: "", nourish: "", connect: "" }
  );
  const [saved, setSaved] = useState(false);
  const lifeScore = Math.round((Object.values(ratings).reduce((s,r) => s+r,0)/5)*20);

  function save() {
    onSave({ id: existing?.id || Date.now().toString(), date: todayStr(), ratings, notes });
    setSaved(true);
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 12 }}>
          {(Object.entries(CATEGORIES) as [CategoryId, typeof CATEGORIES[CategoryId]][]).map(([id, cat]) => (
            <div key={id} style={{ textAlign: "center" as const }}>
              ...
              <div style={{ fontSize: 12, fontWeight: 700, color: RATINGS[ratings[id]].color }}>{ratings[id]}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div key={catId} style={{ ...S.card, marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
            {([1,2,3,4,5] as Rating[]).map(r => (
              <button key={r} onClick={() => setRatings(p => ({ ...p, [catId]: r }))} style={{
                flex: 1, padding: "9px 0", borderRadius: 10, fontWeight: 700, fontSize: 11,
                cursor: "pointer", fontFamily: "inherit",
                background: ratings[catId] === r ? RATINGS[r].bg : C.bg,
                color: ratings[catId] === r ? RATINGS[r].color : C.muted,
                border: `2px solid ${ratings[catId] === r ? RATINGS[r].color : C.border}`,
              }}>{RATINGS[r].label.split(" ")[0]}</button>
            ))}
          </div>
          <input style={{ ...S.input, fontSize: 13 }} placeholder={`Notes on ${cat.label.toLowerCase()} today... (optional)`}
            value={notes[catId]} onChange={e => setNotes(p => ({ ...p, [catId]: e.target.value }))} />
        </div>
      ))}

      <button onClick={save} style={{ ...btn("primary"), width: "100%", justifyContent: "center", padding: "16px", fontSize: 16 }}>
...
// ─── History Calendar ─────────────────────────────────────────────────────────
function HistoryTab({ habits, habitLogs, meals, dayRatings }: {
  habits: Habit[]; habitLogs: HabitLog[]; meals: Meal[]; dayRatings: DayRating[];
}) {
  const now = new Date();
  ...
  // All dates with any data
  function hasData(dateStr: string): boolean {
    return habitLogs.some(l => l.date === dateStr) ||
      meals.some(m => m.date === dateStr) ||
      dayRatings.some(r => r.date === dateStr);
  }

  function getScore(dateStr: string): number | null {
    const r = dayRatings.find(d => d.date === dateStr);
    if (!r) return null;
    return Math.round((Object.values(r.ratings).reduce((s, v) => s + v, 0) / 5) * 20);
  }
  ...
  const detail = selectedDate ? {
    rating: dayRatings.find(r => r.date === selectedDate),
    logs: habitLogs.filter(l => l.date === selectedDate),
    food: meals.filter(m => m.date === selectedDate && !m.isBeverage),
    bevs: meals.filter(m => m.date === selectedDate && m.isBeverage),
  } : null;
    ...
    <div>
      ...
      <div style={S.card}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <button onClick={prevMonth} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: C.navy, padding: "4px 8px" }}>‹</button>
          <div style={{ fontWeight: 800, fontSize: 16 }}>{monthName}</div>
          <button onClick={nextMonth} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: calYear === now.getFullYear() && calMonth === now.getMonth() ? C.border : C.navy, padding: "4px 8px" }}>›</button>
        </div>

        {/* Day headers */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", marginBottom: 6 }}>
          {["M","T","W","T","F","S","S"].map((d, i) => (
            <div key={i} style={{ textAlign: "center" as const, fontSize: 11, fontWeight: 700, color: C.muted, padding: "4px 0" }}>{d}</div>
          ))}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4 }}>
          {cells.map((day, i) => {
            ...
            const data = !future && hasData(dateStr);
            const score = !future ? getScore(dateStr) : null;

            return (
              <button key={i} onClick={() => !future && setSelectedDate(selected ? null : dateStr)}
                style={{
                  padding: "8px 0", borderRadius: 10, border: "none", cursor: future ? "default" : "pointer",
                  background: selected ? C.navy : today ? C.goldLight : C.bg,
                  color: future ? C.border : selected ? "#fff" : today ? C.navy : C.text,
                  fontWeight: today || selected ? 800 : 500, fontSize: 13,
                  display: "flex", flexDirection: "column", alignItems: "center", gap: 3,
                  fontFamily: "inherit",
                }}>
                {day}
                {data && (
                  <div style={{ width: 6, height: 6, borderRadius: "50%", background: selected ? "#fff" : score !== null ? (score >= 80 ? C.mint : score >= 60 ? C.gold : C.coral) : C.muted }} />
                )}
              </button>
            );
          })}
      ...
      {selectedDate && detail && (
        <div>
          ...
          {detail.rating ? (
            <div style={S.card}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <div style={{ fontWeight: 900, fontSize: 24, color: C.gold }}>
                  {Math.round((Object.values(detail.rating.ratings).reduce((s,r) => s+r,0)/5)*20)}/100
                </div>
              </div>
              {(Object.entries(CATEGORIES) as [CategoryId, typeof CATEGORIES[CategoryId]][]).map(([catId, cat]) => {
                const r = detail.rating!.ratings[catId];
                const note = detail.rating!.notes[catId];
                return (
                  <div key={catId} style={{ marginBottom: 10 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      ...
                      <div style={{ display: "flex", gap: 3, marginLeft: "auto" }}>
                        {[1,2,3,4,5].map(n => <div key={n} style={{ width: 10, height: 10, borderRadius: "50%", background: n <= r ? cat.color : C.border }} />)}
                      </div>
                      <span style={{ fontSize: 12, fontWeight: 700, color: RATINGS[r].color }}>{RATINGS[r].label}</span>
                    </div>
            ...
            <div style={{ ...S.card, textAlign: "center", color: C.muted, padding: 16 }}>
              <div style={{ fontSize: 13 }}>No check-in recorded for this day</div>
            </div>
          )}
          ...
          {detail.food.length > 0 && (
            <div style={S.card}>
              <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 12 }}>🍽️ Food</div>
              {detail.food.map(meal => {
                const tag = TAG_CONFIG[meal.tag];
                return (
                  <div key={meal.id} style={{ padding: "8px 0", borderBottom: `1px solid ${C.border}` }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                      ...
                      <span style={{ fontSize: 11, color: C.muted, background: C.bg, borderRadius: 8, padding: "1px 6px" }}>{PORTION_CONFIG[meal.portion]}</span>
                    </div>
                    <div style={{ fontSize: 13, color: C.text }}>{meal.description}</div>
            ...
            <div style={S.card}>
              ...
              {detail.bevs.map(bev => (
                <div key={bev.id} style={{ padding: "8px 0", borderBottom: `1px solid ${C.border}` }}>
                  ...
                  <div style={{ fontSize: 13, color: C.text }}>{bev.description}</div>
          ...
          {!detail.rating && detail.logs.length === 0 && detail.food.length === 0 && detail.bevs.length === 0 && (
            <div style={{ ...S.card, textAlign: "center", color: C.muted, padding: 24 }}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>📭</div>
              <div style={{ fontWeight: 700 }}>Nothing logged this day</div>
            </div>
          )}
        </div>
      )}
      ...
      {!selectedDate && (
        <div style={{ textAlign: "center", color: C.muted, fontSize: 13, padding: "16px 0" }}>
          Tap any date to see what happened that day
        </div>
      )}
    </div>
  );
...
// ─── Analytics Tab ────────────────────────────────────────────────────────────
function AnalyticsTab({ habits, habitLogs, meals, dayRatings }: {
  habits: Habit[]; habitLogs: HabitLog[]; meals: Meal[]; dayRatings: DayRating[];
}) {
  const [period, setPeriod] = useState<7|30>(7);
  const dates = getLastNDates(period);
  const scoreData = dates.map(date => {
    const r = dayRatings.find(d => d.date === date);
    const score = r ? Math.round((Object.values(r.ratings).reduce((s,v) => s+v,0)/5)*20) : null;
    return { date, score, label: new Date(date+"T12:00:00").toLocaleDateString("en", { weekday: "short" }) };
  });
  ...
  const catAverages = (Object.keys(CATEGORIES) as CategoryId[]).map(catId => {
    const rs = dayRatings.filter(r => dates.includes(r.date)).map(r => r.ratings[catId]);
    return { catId, avg: rs.length ? Math.round(rs.reduce((s,r) => s+r,0)/rs.length*10)/10 : 0 };
  });
  const weekHabits = habits.filter(h => h.active).map(h => {
    const hits = getWeeklyHits(h, habitLogs);
    const target = getFrequencyTarget(h.frequency);
    return { habit: h, hits, target, pct: Math.min(100, Math.round((hits/target)*100)) };
  });
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 6, background: C.card, borderRadius: 12, padding: 4, marginBottom: 16, border: `1px solid ${C.border}` }}>
        {([7,30] as const).map(p => (
          <button key={p} onClick={() => setPeriod(p)} style={{ flex: 1, padding: "9px 0", borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit", background: period === p ? C.gold : "transparent", color: period === p ? C.navy : C.muted, border: "none" }}>Last {p} days</button>
        ))}
      </div>
      ...
      <div style={S.card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          {avgScore !== null && <div style={{ fontWeight: 900, fontSize: 20, color: C.gold }}>{avgScore}<span style={{ fontSize: 12, color: C.muted }}>/100</span></div>}
        </div>
      ...
      <div style={S.card}>
        ...
        {weekHabits.map(({ habit, hits, target, pct }) => {
          const cat = CATEGORIES[habit.category];
          return (
...
function FoodTab({ meals, onAdd, onDelete, calorieItems }: { meals: Meal[]; onAdd: (m: Omit<Meal,"id">) => void; onDelete: (id: string) => void; calorieItems: CalorieItem[] }) {
  ...
  const [form, setForm] = useState({ mealType: "Breakfast", description: "", tag: "whole" as MealTag, portion: "single" as Portion, calories: "", time: nowStr() });
  ...
  function submit() {
    ...
    onAdd({ mealType: form.mealType, description: form.description, tag: form.tag, portion: form.portion, calories: form.calories ? Number(form.calories) : undefined, time: form.time, date: todayStr(), isBeverage: false });
    setForm({ mealType: "Breakfast", description: "", tag: "whole", portion: "single", calories: "", time: nowStr() });
    setShowForm(false);
  }
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
        <div style={{ ...S.card, textAlign: "center", background: C.coralLight, marginBottom: 0, padding: "12px 8px" }}>
          <div style={{ fontSize: 24, fontWeight: 900 }}>{snacks}</div>
          <div style={{ fontSize: 10, color: C.muted }}>Snacks 🍿</div>
        </div>
        <div style={{ ...S.card, textAlign: "center", background: junkPkg > 1 ? C.coralLight : C.greenLight, marginBottom: 0, padding: "12px 8px" }}>
          <div style={{ fontSize: 24, fontWeight: 900, color: junkPkg > 1 ? C.coral : C.navy }}>{junkPkg}</div>
          <div style={{ fontSize: 10, color: C.muted }}>Junk/Pkg 🍔</div>
        </div>
        <div style={{ ...S.card, textAlign: "center", background: C.goldLight, marginBottom: 0, padding: "12px 8px" }}>
          <div style={{ fontSize: 24, fontWeight: 900 }}>{totalCals || "—"}</div>
          <div style={{ fontSize: 10, color: C.muted }}>Calories 🔥</div>
        </div>
      </div>
      {junkPkg > 1 && <div style={{ background: C.coralLight, borderRadius: 12, padding: "10px 14px", marginBottom: 12, fontSize: 13, color: C.coral, fontWeight: 600 }}>⚠️ {junkPkg} junk/packaged meals today!</div>}
      ...
      {showForm && (
        <div style={{ ...S.card, border: `2px solid ${C.gold}`, marginBottom: 16 }}>
          ...
          <input style={{ ...S.input, marginBottom: 12 }} placeholder="e.g. Idli sambar..." value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} />
      ...
      <div style={S.sectionTitle}>Today's Food</div>
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
        <div style={{ ...S.card, textAlign: "center", background: C.goldLight, marginBottom: 0 }}>
          <div style={{ fontSize: 28, fontWeight: 900 }}>{coffeeCount}</div>
          <div style={{ fontSize: 11, color: C.muted }}>Coffee/Tea ☕</div>
        </div>
        <div style={{ ...S.card, textAlign: "center", background: C.mintLight, marginBottom: 0 }}>
          <div style={{ fontSize: 28, fontWeight: 900 }}>{totalCals || "—"}</div>
          <div style={{ fontSize: 11, color: C.muted }}>Bev cals 🔥</div>
        </div>
      </div>
      ...
      <div style={S.sectionTitle}>Today's Drinks</div>
      ...
      {todayBevs.map(bev => (
        <div key={bev.id} style={{ ...S.card, display: "flex", alignItems: "center", gap: 12, padding: "12px 14px" }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 2 }}>{bev.mealType}</div>
            <div style={{ fontSize: 14, marginBottom: 2 }}>{bev.description}</div>
            <div style={{ fontSize: 12, color: C.muted }}>{bev.time}{bev.calories ? ` · ${bev.calories} cal` : ""}</div>
          </div>
...
function CalorieTab({ items, onUpdate }: { items: CalorieItem[]; onUpdate: (items: CalorieItem[]) => void }) {
  ...
  function save() {
    ...
    if (editId) { onUpdate(items.map(i => i.id === editId ? { ...i, name: form.name, emoji: form.emoji, calories: Number(form.calories), category: form.category } : i)); setEditId(null); }
    else onUpdate([...items, { id: "c"+Date.now(), name: form.name, emoji: form.emoji, calories: Number(form.calories), category: form.category, isCustom: true }]);
    setForm({ name: "", emoji: "🍽️", calories: "", category: "Common Foods" });
    setShowAdd(false);
  }
        ...
        <div key={cat} style={{ marginBottom: 16 }}>
          ...
          {catItems.map(item => (
            <div key={item.id} style={{ ...S.card, display: "flex", alignItems: "center", gap: 12, padding: "11px 14px" }}>
              ...
              {item.isCustom && <button onClick={() => onUpdate(items.filter(i => i.id !== item.id))} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 14, color: C.coral }}>✕</button>}
            </div>
          ))}
        </div>
      ))}
...
function SettingsTab({ habits, onUpdate }: { habits: Habit[]; onUpdate: (h: Habit[]) => void }) {
  const [showAdd, setShowAdd] = useState(false);
  const [form, setForm] = useState({ title: "", emoji: "⭐", type: "checkbox" as HabitType, category: "move" as CategoryId, frequency: "daily" as Frequency, allowComments: false });
  const EMOJIS = ["⭐","💪","📚","🧘","🏃","💧","🎯","🌿","🛁","💊","🎨","🎵","🚴","📞","🎾","🏋️","🤸"];
  const FREQS: Frequency[] = ["daily","weekdays","weekends","2x_week","3x_week","weekly"];

  function addHabit() {
    if (!form.title.trim()) return;
    onUpdate([...habits, { id: "h"+Date.now(), ...form, active: true }]);
    setForm({ title: "", emoji: "⭐", type: "checkbox", category: "move", frequency: "daily", allowComments: false });
    setShowAdd(false);
  }
    ...
    <div>
      ...
      {showAdd && (
        <div style={{ ...S.card, border: `2px solid ${C.gold}`, marginBottom: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "52px 1fr", gap: 8, marginBottom: 10 }}>
            <div><label style={S.label}>Icon</label><select value={form.emoji} onChange={e => setForm(p => ({ ...p, emoji: e.target.value }))} style={{ ...S.input, fontSize: 18, padding: "8px 4px" }}>{EMOJIS.map(e => <option key={e} value={e}>{e}</option>)}</select></div>
            <div><label style={S.label}>Name</label><input style={S.input} placeholder="e.g. Tennis" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} /></div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
            <div>
              <label style={S.label}>Category</label>
              <select style={S.input} value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value as CategoryId }))}>
                {(Object.entries(CATEGORIES) as [CategoryId, typeof CATEGORIES[CategoryId]][]).map(([id, cat]) => <option key={id} value={id}>{cat.emoji} {cat.label}</option>)}
              </select>
            </div>
            <div>
              <label style={S.label}>Frequency</label>
              <select style={S.input} value={form.frequency} onChange={e => setForm(p => ({ ...p, frequency: e.target.value as Frequency }))}>
                {FREQS.map(f => <option key={f} value={f}>{FREQUENCY_LABELS[f]}</option>)}
              </select>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            {(["checkbox","time"] as HabitType[]).map(t => (
              <button key={t} onClick={() => setForm(p => ({ ...p, type: t }))} style={{ flex: 1, padding: "9px 0", borderRadius: 10, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "inherit", background: form.type === t ? C.navy : C.bg, color: form.type === t ? "#fff" : C.muted, border: `1.5px solid ${form.type === t ? C.navy : C.border}` }}>
                {t === "checkbox" ? "✓ Done/Not done" : "🕐 Log time"}
              </button>
            ))}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", marginBottom: 12, borderTop: `1px solid ${C.border}` }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: C.muted }}>Show a notes field when logging</div>
            </div>
            <button onClick={() => setForm(p => ({ ...p, allowComments: !p.allowComments }))} style={{ padding: "6px 14px", borderRadius: 20, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "inherit", background: form.allowComments ? C.mintLight : C.bg, color: form.allowComments ? C.mint : C.muted, border: `1.5px solid ${form.allowComments ? C.mint : C.border}` }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: C.muted }}>
                      {FREQUENCY_LABELS[habit.frequency]} · {habit.type === "checkbox" ? "✓" : "🕐"}
                      {habit.allowComments ? " · 💬 comments" : ""}
                    </div>
                  </div>
                  <button onClick={() => onUpdate(habits.map(h => h.id === habit.id ? { ...h, allowComments: !h.allowComments } : h))} style={{ padding: "4px 8px", borderRadius: 12, fontWeight: 700, fontSize: 10, cursor: "pointer", fontFamily: "inherit", background: habit.allowComments ? C.purpleLight : C.bg, color: habit.allowComments ? C.purple : C.muted, border: `1px solid ${habit.allowComments ? C.purple : C.border}` }}>💬</button>
                  <button onClick={() => onUpdate(habits.map(h => h.id === habit.id ? { ...h, active: !h.active } : h))} style={{ padding: "5px 10px", borderRadius: 20, fontWeight: 700, fontSize: 11, cursor: "pointer", fontFamily: "inherit", background: habit.active ? C.mintLight : C.bg, color: habit.active ? C.mint : C.muted, border: `1.5px solid ${habit.active ? C.mint : C.border}` }}>{habit.active ? "On" : "Off"}</button>
                  <button onClick={() => onUpdate(habits.filter(h => h.id !== habit.id))} style={{ background: "none", border: "none", cursor: "pointer", color: C.coral, fontSize: 15 }}>✕</button>
                </div>
              </div>
            ))}
          </div>
...
export default function LifeTracker() {
  ...
  function saveHabitLog(habitId: string, value: string, comment?: string) {
    ...
    if (existing) {
      ...
      setHabitLogs(p => [...p, { id: Date.now().toString(), habitId, value, comment, date: todayStr() }]);
    }
  }

  function removeHabitLog(habitId: string) {
    setHabitLogs(p => p.filter(l => !(l.habitId === habitId && l.date === todayStr())));
  }

  function saveDayRating(rating: DayRating) {
    setDayRatings(p => {
      const exists = p.find(r => r.date === rating.date);
      return exists ? p.map(r => r.date === rating.date ? rating : r) : [...p, rating];
    });
  }
  ...
  const TITLES: Record<View, string> = {
    login: "", today: "My Day", checkin: "Evening Check-in",
    food: "Food Log", beverages: "Beverages", calories: "Calorie Reference",
    analytics: "Analytics", history: "History", settings: "Habits & Settings",
  };
  ...
  return (
    <div style={S.app}>
      <div style={S.header}>
        ...
        <button onClick={() => setLoggedIn(false)} style={{ background: "none", border: "none", color: "#a0aec0", cursor: "pointer", fontSize: 18 }}>↩</button>
      </div>
      ...
      <div style={S.body}>
        ...
        {view === "checkin"   && <CheckinTab existing={todayRating} onSave={saveDayRating} />}
        {view === "food"      && <FoodTab meals={meals} onAdd={m => setMeals(p => [...p, { ...m, id: Date.now().toString() }])} onDelete={id => setMeals(p => p.filter(m => m.id !== id))} calorieItems={calorieItems} />}
        {view === "beverages" && <BeveragesTab meals={meals} onAdd={m => setMeals(p => [...p, { ...m, id: Date.now().toString() }])} onDelete={id => setMeals(p => p.filter(m => m.id !== id))} calorieItems={calorieItems} />}
